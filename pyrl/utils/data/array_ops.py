...
from .converter import index_to_slice
...
def data_ptr(item):
    if is_num(item):
        ...
        return item.__array_interface__["data"][0]
    elif is_torch(item):
        return item.data_ptr()
    return None


def merge_axes(item, axes):
    if not is_arr(item):
        return item
    axes = list(np.arange(item.ndim)[axes])
    axes = index_to_slice(axes)
    assert isinstance(axes, slice)
    item_shape = list(item.shape)
    new_shape = np.prod(item_shape[axes])
    start_indices = [] if axes.start is None else item_shape[: axes.start]
    stop_indices = [] if axes.stop is None else item_shape[axes.stop :]
    new_shape = start_indices + [new_shape] + stop_indices
    return item.reshape(new_shape)


def split_axis(item, axis, newaxes):
    if is_arr(item) and len(newaxes) > 1:
        axis = axis % item.ndim
        axis = axis if axis >= 0 else item.ndim + axis
...
def move_axis(item, axis, target_axis):
    if is_arr(item):
        if is_np_arr(item):
            return np.moveaxis(item, axis, target_axis)
        else:
            import torch

            return torch.movedim(item, axis, target_axis)
    return item
...
def to_cpu(x, non_blocking=True):
    import torch
    ...
    if isinstance(x, torch.Tensor):
        if non_blocking == "raw":
            ...
            x = x.to("cpu", non_blocking=non_blocking)
    return x


def to_cuda(x, device="cuda", non_blocking=True):
    import torch
...
def expand_to_ndim(item, ndim, used_dim):
    if not is_arr(item):
        return item
    assert item.ndim == len(used_dim), "used_dim should match the dimension of the array!"
    used_dim = np.sort(used_dim)

    i, j = 0, 0
    while i < ndim:
        if j < len(used_dim) and used_dim[j] == i:
            j += 1
            i += 1
            continue
        item = unsqueeze(item, i)
        i += 1
    return item


def gather_index(item, axis, index, batch_dims):
    if not (is_arr(item) and is_arr(index)):
        return index
    if batch_dims is None:
        batch_dims = list(range(index.ndim))
    elif is_num(batch_dims):
        batch_dims = [batch_dims]
    assert axis in batch_dims, f"axis should be in batch_dims {batch_dims}"
    batch_dims = [_ if _ >= 0 else _ + item.ndim for _ in batch_dims]
    axis = axis if axis >= 0 else axis + item.ndim
    for i in range(len(batch_dims)):
        if batch_dims[i] != axis:
            assert index.shape[i] == item.shape[batch_dims[i]], f"Shape error {index.shape}, {batch_dims}, {item.shape}!"

    # print(1, index.shape, index.dtype)
    if index.ndim != item.ndim:
        index = expand_to_ndim(index, item.ndim, batch_dims)
    # print(1.5, index.shape, index.dtype, item.ndim, batch_dims)
    # [torch.Size([1, 1536]), torch.Size([1, 1536])] ['int64', 'bool']
    # 1 torch.Size([1, 1536]) torch.int64
    # 1.5 torch.Size([1, 1, 1, 1, 1536]) torch.int64 3 [0, -1]
    # exit(0)
    index = expand_as(index, item, [axis])
    return index


def gather(item, axis, index, batch_dims=None):
    if not is_arr(item):
        return item
    # print("????")
    index = gather_index(item=item, axis=axis, index=index, batch_dims=batch_dims)

    if is_np_arr(item):
        return np.take_along_axis(item, index, axis)
        ...
        return torch.gather(item, axis, index)
    return item
...
def allreduce(item, op="MEAN", device="cuda"):
    ...
    from pyrl.utils.data import as_dtype
...
def copy_(item, other, index=None, other_index=None, non_blocking=True):
    # print(not is_arr(item), not (is_arr(other) or is_num(other)), is_num(other))
    if not is_arr(item) and not (is_arr(other) or is_num(other)):
        return

    # print(is_arr(item), is_arr(other) and not is_num(other))

    # print(item.shape)
    other = other if other_index is None else other[index_to_slice(other_index)]
    index = slice(None) if index is None else index_to_slice(index)

    if is_np(item) or is_num(other):
        assert data_ptr(item) != data_ptr(other), "You need to deepcopy source data first, when source and target share the same memory!"
        item[index] = other
    elif is_torch(item):
        assert data_ptr(item) != data_ptr(other), "You need to deepcopy source data first, when source and target share the same memory!"
        if is_torch(other) and isinstance(index, slice):
            # copy_ is faster!!
            item[index].copy_(other, non_blocking=non_blocking)
        else:
            if is_np(other):
                import torch
...
def concat_torch_distribution(item, axis=0):
    ...
    from pyrl.utils.torch import CustomIndependent, ScaledTanhNormal, ScaledNormal
    import torch
...
def sample_and_pad(n, num=1200, np_random=None, pad=True):
