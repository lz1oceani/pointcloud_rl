agent_cfg = dict(
    ...
    critic_cfg=dict(
        ...
        nn_cfg=dict(
            ...
            visual_nn_cfg=None,
            mlp_cfg=dict(
                type="LinearMLP", norm_cfg=None, mlp_spec=["128 + agent_shape + action_shape", 1024, 1024, 1], bias=True, inactivated_output=True
            ),
        ),
    ...
    svea=False,
    obs_aug=[
        dict(type="RandomCrop", size=[50, 125], padding=4, padding_mode="edge", req_keys=["rgb", "depth", "seg"], main_key="rgb", use_kornia=False),
    ],
)
