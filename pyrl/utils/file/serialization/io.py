...
def load(file, file_format=None, **kwargs):
    ...
    if isinstance(file, bytes):
        file = BytesIO(file)
    if is_str(file):
        if file.startswith("gl_db:"):
            from pyrl.utils.external import gl_db

            return gl_db[file[len("gl_db:") :]]
...
def dump(obj, file=None, file_format=None, **kwargs):
    if isinstance(file, Path):
        file = str(file)
    if is_str(file) and file.startswith("gl_db:"):
        from pyrl.utils.external import gl_db

        gl_db[file[len("gl_db:")]] = obj
        return
    if file_format is None:
        if is_str(file):
            file_format = file.split(".")[-1]
