...
def to_torch(
    ...
    if item_dtype in dtype_torch_map and dtype is None:
        # dtype = dtype_torch_map[item_dtype]
        item = as_dtype(item, dtype_torch_map[item_dtype])

    if equal(item_dtype, dtype):
        dtype = None
...
def index_to_slice(index):
    if isinstance(index, slice) or is_num(index):
        return index
    elif len(index) == 1:
        index = index[0]
