...
from .array_ops import (
    ...
    copy_,
    pin_memory,
    ops_single_torch_distribution,
)
from .converter import as_dtype, to_np, to_torch, slice_to_range, to_array
from .type_utils import get_dtype, is_list_of, is_dict, is_h5, is_arr, is_num, is_np, is_str, is_torch_distribution
...
class GDict:
    ...
    def _recursive_do_on_memory(cls, memory, function, new=True, ignore_list=False, *args, **kwargs):
        ...
        def _execute(value):
            if cls._is_final(value):
                ...
                return cls._recursive_do_on_memory(value, function, new, ignore_list, *args, **kwargs)

        if isinstance(memory, dict):
            ret = {} if new else memory
            for key, value in memory.items():
                ret[key] = _execute(value)
            return ret
            ...
            for key, value in enumerate(memory):
                ret[key] = _execute(value)
            return ret
        elif is_torch_distribution(memory):
            return ops_single_torch_distribution(memory, function, *args, **kwargs)
        else:
    ...
    def get_attr(self, name, default=None, wrapper=True):
        return self._recursive_do(self.memory, lambda item: getattr(item, name, default), name=name, default=default, wrapper=wrapper)

    def pin_memory(self, wrapper=True):
        return self._recursive_do(self.memory, pin_memory, wrapper=wrapper)

    def to_hdf5(self, file):
        from pyrl.utils.file import dump_hdf5

        dump_hdf5(self.memory, file)

    @property
    def data_ptr(self, wrapper=True):
        return self._recursive_do(self.memory, data_ptr, wrapper=wrapper)

    @property
    def data_ptr(self, wrapper=True):
        return self._recursive_do_gdict(self.memory, data_ptr, wrapper=wrapper)
    ...
    def cpu(self, non_blocking=True, wrapper=True):
        return self._recursive_do(self.memory, to_cpu, non_blocking=non_blocking, wrapper=wrapper)

    def cuda(self, device="cuda", non_blocking=True, wrapper=True):
        return self._recursive_do(self.memory, to_cuda, device=device, non_blocking=non_blocking, wrapper=wrapper)
    ...
    def merge_axes(self, axes, wrapper=True):
        assert isinstance(axes, (list, tuple, slice))
        return self._recursive_do(self.memory, merge_axes, axes=axes, wrapper=wrapper, capacity=None)

    def split_axis(self, axis, newaxes, wrapper=True):
        assert isinstance(newaxes, (list, tuple))
        return self._recursive_do(
            self.memory, split_axis, axis=axis, newaxes=newaxes, wrapper=wrapper, capacity=self.capacity if axis != 0 else newaxes[0]
        )

    def move_axis(self, axis, target_axis, wrapper=True):
        return self._recursive_do(
            self.memory, move_axis, axis=axis, target_axis=target_axis, wrapper=wrapper, capacity=None
    ...
    @classmethod
    def _assign(cls, memory, source, indices=None, source_indices=None, ignore_list=False, non_blocking=False, print_key=""):
        if isinstance(source, tuple):
            source = list(source)

        if is_dict(memory):
            assert is_dict(source), f"{type(memory), type(source)}"
            for key in memory:
                if key not in source:
                    continue
                source_key = source if np.isscalar(source) else source[key]
                memory[key] = cls._assign(memory[key], source_key, indices, source_indices, ignore_list, non_blocking, print_key + f"/{key}")
        elif is_arr(memory):
            assert is_arr(source) or np.isscalar(source), f"{type(memory), type(source)}"
            if share_memory(memory, source):
                source = deepcopy(source)
            copy_(memory, source, indices, source_indices, non_blocking)
        elif is_list_of(memory):
            if ignore_list:
                memory[indices] = source[source_indices]
            else:
                num = len(memory)
                if hasattr(source, "__len__"):
                    num = min(num, len(source))
                for i in range(num):
                    source_i = source if np.isscalar(source) else source[i]
                    memory[i] = cls._assign(memory[i], source_i, indices, source_indices, ignore_list, non_blocking, print_key + f"/{i}")
        return memory

    def assign(self, indices, source, source_indices=None, non_blocking=False, wrapper=True):
        source = source.memory if isinstance(source, GDict) else source
        self.memory = self._assign(self.memory, source, indices, source_indices, non_blocking=non_blocking)
        return self if wrapper else self.memory

    def assign_all(self, source, non_blocking=False, wrapper=True):
        self.assign(None, source, non_blocking=non_blocking)
        return self if wrapper else self.memory

    def assign_list(self, index, source, source_indices=None):
        source = source.memory if isinstance(source, GDict) else source
        assert is_num(index)
        self.memory = self._assign(self.memory, source, index, source_indices, ignore_list=True)
...
class DictArray(GDict):
    ...
    def assert_shape(cls, memory, capacity):
        assert cls.check_shape(memory, capacity), f"The item has shape: {memory.shape} and the first dimension is not {capacity}!"
    ...
    def gather(self, axis, index, batch_dims, wrapper=True):
        return self._recursive_do(self.memory, gather, axis=axis, index=index, batch_dims=batch_dims, wrapper=wrapper)
